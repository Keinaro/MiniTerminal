...
#include <cstdlib>
#include <cerrno>
// Librerías de C necesarias para interactuar con el Sistema Operativo
#ifdef _WIN32
#include <process.h>
#else
#include <unistd.h>
...
void ejecutar_comando(const std::vector<std::string>& args_str) {
    if (args_str.empty()) return; // Si el usuario solo presiona Enter, no hacemos nada

    // Fase 5: Memoria Dinámica[cite: 77]. 
    // Reservamos memoria usando 'new' [cite: 31] para un arreglo de punteros que requiere execvp
    char** args = new char*[args_str.size() + 1];

    for (size_t i = 0; i < args_str.size(); ++i) {
        // Reservamos memoria para cada palabra del comando [cite: 79]
...
#ifdef _WIN32
    // En Windows usamos _spawnvp para ejecutar y esperar al proceso hijo.
    ...
    delete[] args_win;
#else
    // Creamos el proceso hijo con fork() [cite: 24]
    pid_t pid = fork();

    if (pid == -1) {
        std::cerr << "Error al crear el proceso (fork): " << std::strerror(errno) << "\n";
    } else if (pid == 0) {
        // Estamos en el proceso hijo, ejecutamos el comando con exec() [cite: 26, 56]
    ...
    // Fase 5: Liberar la memoria para evitar fugas (memory leaks) [cite: 80, 82]
    for (size_t i = 0; i < args_str.size(); ++i) {
        delete[] args[i];
    }
    delete[] args;
...
int main() {
    ...
    while (true) {
        // Mostrar el prompt [cite: 47]
        std::cout << "MiniTerminal_SyJ> ";
        
        // Leer el comando desde el teclado [cite: 48]
        ...
        // Usamos stringstream para separar la línea por espacios fácilmente
        std::vector<std::string> args_str;
        std::istringstream iss(linea);
        std::string token;
        
        while (iss >> token) {
            args_str.push_back(token); // Guardamos cada parte (ej. "ls", "-l", "/home") [cite: 63, 64]
        }
